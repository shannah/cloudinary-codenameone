...
public class Transformation {
	...
	public Transformation border(int width, String color) {
		return param("border", "" + width + "px_solid_" + new CN1String(color).replaceFirst("^#", "rgb:"));
	}
	...
	public Transformation offset(float[] value) {
		if (value.length < 2) throw new IllegalArgumentException("Offset range must include at least 2 items");
		Float[] numberArray = new Float[]{value[0], value[1]};
		return offset(numberArray);
	}
	
	public Transformation offset(double[] value) {
		if (value.length < 2) throw new IllegalArgumentException("Offset range must include at least 2 items");
		Double[] numberArray = new Double[]{value[0], value[1]};
		return offset(numberArray);
	}
	
	public Transformation offset(Integer[] value) {
		if (value.length < 2) throw new IllegalArgumentException("Offset range must include at least 2 items");
		return param("offset", value);
	}
	...
	public Transformation videoSamplingSeconds(Integer value) {
	...
	public String generate(Map options) {
		...
		if (size != null) {
			String[] size_components = new CN1String(size).split("x");
		...
		if (background != null) {
			background = new CN1String(background).replaceFirst("^#", "rgb:");
		}

		String color = (String) options.get("color");
		if (color != null) {
			color = new CN1String(color).replaceFirst("^#", "rgb:");
		}
	...
	private static String[] splitRange(Object range) {
		...
		} else if (range instanceof Double[] && ((Double[]) range).length >= 2) {
			Double[] numberArrayRange = ((Double[]) range);
			return new String[]{numberArrayRange[0].toString(), numberArrayRange[1].toString()};
		} else if (range instanceof Float[] && ((Float[]) range).length >= 2) {
			Float[] numberArrayRange = ((Float[]) range);
			return new String[]{numberArrayRange[0].toString(), numberArrayRange[1].toString()};
		}else if (range instanceof Integer[] && ((Integer[]) range).length >= 2) {
			Integer[] numberArrayRange = ((Integer[]) range);
			return new String[]{numberArrayRange[0].toString(), numberArrayRange[1].toString()};
		}else if (range instanceof String && RANGE_RE.matcher((String) range).matches()) {
			return new CN1String(((String) range)).split("\\.\\.", 2);
		} else {
			return null;
		}
	}
