...
public class Configuration {
    ...
    private static Configuration parseConfigUrl(String cloudinaryUrl) {
        ...
        if (cloudinaryUri.getUserInfo() != null) {
            String[] creds = new CN1String(cloudinaryUri.getUserInfo()).split(':');
        ...
        if (cloudinaryUri.getQuery() != null) {
            for (String param : new CN1String(cloudinaryUri.getQuery()).split('&')) {
                String[] keyValue = new CN1String(param).split('=');
                String val = null;
                try {
//                    params.put(keyValue[0], URLDecoder.decode(keyValue[1], "ASCII"));
                    val = URLDecoder.decode(keyValue[1], "ASCII");
                } catch (UnsupportedEncodingException e) {
                    throw new IllegalArgumentException("Error decoding cloudinaryUrl");
                }
