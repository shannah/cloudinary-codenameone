...
public class Url {
	...
	public String generate(String source) {
		...
		if (signUrl) {
			...
			toSign = new CN1String.Builder(toSign).replaceAll("^/+", "").replaceAll("([^:])\\/+", "$1/").toString();
		...
		return new CN1String(StringUtils.join(new String[] { prefix, finalResourceType, signature, transformationStr, version, source}, "/")).replaceAll("([^:])\\/+", "$1/");
	}
	...
	private String[] finalizeSource(String source, String format, String urlSuffix) {
		...
		if (new CN1String(source.toLowerCase()).matches("^https?:/")) {
			...
			try {
				source = SmartUrlEncoder.encode(URLDecoder.decode(new CN1String(source).replace("+", "%2B"), "UTF-8"));
	...
	public String unsignedDownloadUrlPrefix(String source, String cloudName, boolean privateCdn, boolean cdnSubdomain, Boolean secureCdnSubdomain, String cname, boolean secure, String secureDistribution) {
		...
		if (this.config.secure) {
			...
			if (secureCdnSubdomain!=null && secureCdnSubdomain==true) {
				secureDistribution = new CN1String(this.config.secureDistribution).replace("res.cloudinary.com", "res-" + shard(source) + ".cloudinary.com");
			}
